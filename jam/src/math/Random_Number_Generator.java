...
public class Random_Number_Generator {
	...
	public static void main(String[] args) {
		// A naive solution is to create a array with freq[0] number of arr[0]
		// and
		// freq[1] number of arr[1]... in it. Then randomly pick a number from
		// [0, len -1]
		// with uniform distribution. BUT it comsumes a lot of memory because
		// there might
		...
		// Uniformly generate a token within [0, 135), find out with range it
		// falls in.
		RandomNumberGenerator gen = new RandomNumberGenerator(new int[] { 1, 2,
				3, 4 }, new int[] { 10, 5, 20, 100 });
...
class RandomNumberGenerator {
	...
	private int binarySearch(int head, int tail, int token) {
		...
		} else if (this.incre[mid] > token && this.incre[mid - 1] <= token) { // [
																				// this.incre[i],
																				// this.incre[i+1]
																				// )
			return mid;
