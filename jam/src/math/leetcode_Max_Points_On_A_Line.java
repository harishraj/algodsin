...
public class leetcode_Max_Points_On_A_Line {
	...
	public static void main(String[] args) {
		...
		points = new Point[] { new Point(2, 3), new Point(3, 3),
				new Point(-5, 3) };
		maxPoints(points);

		points = new Point[] { new Point(-4, 1), new Point(-7, 7),
				new Point(-1, 5), new Point(9, -25) };
		maxPoints(points);
	}
	...
	public static void linePassMostPoints(Point[] points) {
		...
		for (i = 0; i < n; ++i) {
			...
			for (j = i + 1; j < n; ++j) {
				// System.out.println("Processing" + m[i][0] + ", "+ m[i][1]
				// +", "+ m[j][0] + ", "+ m[j][1]);
		...
		System.out
				.println("The function of line which passes the most number is "
						+ ret);
...
/*
 * Python Version def maxPoints(self, points): Max = 0 for i in
 * xrange(len(points)): count = collections.defaultdict(int) same_points = 1
 * localMax = 0 for j in xrange(i + 1, len(points)): # Calc uniq key to
 * represent line function. dx = points[i].x - points[j].x dy = points[i].y -
 * points[j].y # Make sure dx / gcd > 0 when gcd != 0 gcd = self.gcd(dx, dy) gcd
 * = gcd if dx * dy > 0 else -gcd if gcd == 0: same_points += 1 else: key =
 * '{0}:{1}'.format(dx / gcd, dy / gcd) count[key] += 1 # Update localMax.
 * localMax = max(localMax, count[key]) Max = max(Max, same_points + localMax)
 * return Max
 * 
 * def gcd(self, a, b): return self.gcd(b % a, a) if a != 0 else b
 */
