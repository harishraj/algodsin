...
public class leetcode_4Sum {
	...
	public static void main(String[] args) {
		// for (ArrayList<Integer> i : fourSum(new int[] {2, 1, 0, -1}, 2))
		// System.out.println(i);
		solve(new int[] { 1, 2, 3, 4, 5 }, 10);
	}

	// Hashing pairs: sum/index. arr with length n => n^2 pairs.
	// Sort pairs. Use two pointers find pair of pairs can sum up to target.
	// sort: n^2(lg n^2) = n^2lgn and search: n
	...
	public static void find_four(HashSet<ArrayList<Integer>> set,
			ArrayList<int[]> a, ArrayList<int[]> b, int[] arr) {
		int i, j;
		int[] foo, bar;
	...
	public static ArrayList<ArrayList<Integer>> fourSum(int[] num, int target) {
		...
		for (int i = 0; i < num.length - 3; ++i) {
			if (i > 0 && num[i - 1] == num[i]) {
				// Latter threeSum either has no or the same solution as
				// previous.
				continue;
			}
			int[] arr = Arrays.copyOfRange(num, i + 1, num.length);
			ArrayList<ArrayList<Integer>> threeSum = threeSum(arr, target
					- num[i]);
	...
	public static ArrayList<ArrayList<Integer>> threeSum(int[] num, int target) {
		// Difference between original 3sum question: args num is already
		// sorted.
	...
	// O(N^3), way too slow.
	public static ArrayList<ArrayList<Integer>> fourSumTooSlow(int[] num,
			int target) {
		Arrays.sort(num);
		HashSet<ArrayList<Integer>> all = new HashSet<ArrayList<Integer>>();
...
/*
 * Python Version (AC) class Solution: # @return a list of lists of length 4,
 * [[val1,val2,val3,val4]] def fourSum(self, num, target): if not num or
 * len(num) <= 3: return []
 * 
 * container = set() arr = []
 * 
 * for i in xrange(len(num)): for j in xrange(i + 1, len(num)):
 * arr.append(Token(num[i], num[j], i, j))
 * 
 * d = {} for token in arr: try: d[token.s].append(token) except KeyError:
 * d[token.s] = [token] #print group group = [] for s in sorted(d.keys()):
 * group.append(d[s])
 * 
 * head = 0 tail = len(group) - 1 while head <= tail: if group[head][0].s +
 * group[tail][0].s == target: for h in group[head]: for t in group[tail]: if
 * not h.overlap(t): container.add(':'.join(map(str, sorted(map(lambda x :
 * num[x], [h.idx_a, h.idx_b, t.idx_a, t.idx_b]))))) head += 1 tail -= 1 elif
 * group[head][0].s + group[tail][0].s > target: tail -= 1 else: head += 1
 * 
 ...
 * 
 * def __init__(self, a, b, idx_a, idx_b): self.s = a + b self.idx_a = idx_a
 * self.idx_b = idx_b
 * 
 * def __cmp__(self, that): return self.s - that.s
 * 
 * def overlap(self, that): return set([that.idx_a, that.idx_b]) &
 * set([self.idx_a, self.idx_b])
 */
