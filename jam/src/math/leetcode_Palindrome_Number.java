...
/*
 * Python Version Cheating: def isPalindrome(self, x): x = str(x) head = 0 tail
 * = len(x) - 1 while head < tail: if x[head] != x[tail]: return False head += 1
 * tail -= 1 return True
 * 
 * Not cheating:
 * 
 * def isPalindrome(self, x): if x < 0: return False
 * 
 * left = 1 right = 1
 * 
 * while x / left >= 10: left *= 10
 * 
 * while left > right: if x / left % 10 != x / right % 10: return False left /=
 * 10 right *= 10
 * 
 * return True
 */
