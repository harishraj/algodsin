...
public class leetcode_Valid_Number {
	 ...
	 * Yuk...
	 * 
	 * 1. You are welcome to use regex to cheat. 2. Using state machine. Draw
	 * DFA diagram first.
	 */
	...
	public static boolean isNumber(String s) {
		...
		while (i < s.length()) {
			char cur = s.charAt(i);
			if (cur == '+' || cur == '-') { // more sign must be right after
											// 'e'/'E'
				...
				if (eExist || !firstPart) {
					return false; // Duplicate e detected or trailing e without
									// number part.
				} else {
					eExist = true;
				}
				...
				if (!eExist) {
					firstPart = true; // Digit detected. flag first or second
										// part.
				} else {
					secondPart = true;
				}
		...
		// Must have firstPart, and if not e, return true, if e, must have
		// secondPart.
 ...
 * Python Version
 * 
 * def isNumber(self, s): s = s.strip(); if not s: return False
 * 
 * sign = ('+', '-') e = ('e', 'E') i = 1 if s[0] in sign else 0
 * 
 * has_dot = has_e = first_part = second_part = False
 * 
 * while i < len(s): c = s[i] if c in sign: if not (has_e and s[i - 1] in e):
 * return False elif c == '.': if has_e or has_dot: return False else: has_dot =
 * True elif c in e: if has_e or not first_part: return False else: has_e = True
 * elif c.isdigit(): if not has_e: first_part = True else: second_part = True
 * else: return False
 * 
 * i += 1
 * 
