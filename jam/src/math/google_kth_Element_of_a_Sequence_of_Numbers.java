...
public class google_kth_Element_of_a_Sequence_of_Numbers {
	...
	public static int getKthElement(int index) {
		...
		while (true) {
			digits += 1;
			min = max + 1; // The min of current range is the max of last range
							// + 1.
		...
		String number = Integer.toString(min + (index / digits)); // The number
																	// that
																	// index
																	// falls in.
		int digit = index % digits; // Index of the exact digit should be
									// returned.
