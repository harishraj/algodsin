...
public class leetcode_Permutations_II {
	...
	public static void dfs(int idx, int[] num, boolean[] visited, int[] tokens,
		...
		if (idx == num.length) {
			...
			while (i < num.length) {
				if (visited[i]) {
					...
					// After jump out of the dfs recursion above, if next val in
					// num is same as current one,
					// there is no need to consider any of the elements with the
					// same value in current idx.
 ...
 * Python Version
 * 
 * def permuteUnique(self, num): def dfs(idx, num, tokens, visited, container):
 * if idx == len(num): container.append(tokens[::]) else: i = 0 while i <
 * len(num): if visited[i]: i += 1 else: tokens[idx] = num[i] visited[i] = True
 * dfs(idx + 1, num, tokens, visited, container) visited[i] = False i += 1 while
 * i < len(num) and num[i] == num[i - 1]: i += 1
 * 
 * 
 * if not num or len(num) == 0: return []
 * 
 * num = sorted(num) container = [] tokens = [ 0 ] * len(num) visited = [ False
 * ] * len(num)
 * 
