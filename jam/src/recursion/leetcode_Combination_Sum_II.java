...
public class leetcode_Combination_Sum_II {
	...
	public static void dfs(int i, int target, ArrayList<Integer> acc,
			int[] num, ArrayList<ArrayList<Integer>> container) {
		if (target == 0) {
	...
	public static void nextInt(int[] num, HashSet<ArrayList<Integer>> all,
			ArrayList<Integer> tmp, int sum, int target) {
		for (int i : num)
			System.out.print(i + " ");
		System.out.println();
		if (sum == target) {
			...
			for (int i = 0; i < num.length; ++i) {
				...
				nextInt(Arrays.copyOfRange(num, i + 1, num.length), all, tmp,
						sum + num[i], target);
...
/*
 * Python Version class Solution: # @param candidates, a list of integers #
 * 
 * @param target, integer # @return a list of lists of integers def
 * combinationSum2(self, candidates, target): def dfs(idx, candidates, target,
 * acc, container): if target == 0: container.append(acc[::]) elif target > 0: i
 * = idx while i < len(candidates): acc.append(candidates[i]) dfs(i + 1,
 * candidates, target - candidates[i], acc, container) acc.pop() i += 1
 * 
