...
public class cap_Robot_Walk {
	...
	/**
	 * If all possible paths are asked for, do recursions. If how manay
	 * different paths are there, dp will do.
	 */
	...
	public static void nextGrid(int x, int y, ArrayList<String> path,
	...
	public static void next(int x, int y, ArrayList<String> path,
			ArrayList<ArrayList<String>> all, ArrayList<int[]> offLimits) {
		if (isOffLimit(offLimits, x, y)) {
			return; // If (0, 0) is one of the offLimits then you will have
					// empty result.
		}
		path.add("<" + x + ", " + y + ">");
		if (x == 0 && y == 0) {
