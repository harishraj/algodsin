...
public class leetcode_Combination_Sum {
	...
	/**
	 * Brutal force enumeration. Sort the array first so we can prune when the
	 * rest of values are large than remain.
	 */
	...
	public static void dfs(int i, int target, ArrayList<Integer> acc,
			ArrayList<ArrayList<Integer>> container, int[] candidates) {
		if (target == 0) {
	...
	/*
	 * The solution below is obsolete. HashSet works but does not help reduce
	 * time complexity
	 */
	...
	public static void nextInt(int[] candidates,
			HashSet<ArrayList<Integer>> all, ArrayList<Integer> tmp, int sum,
			int target) {
		if (sum == target) {
 ...
 * Python Version
 * 
 * def combinationSum(self, candidates, target): def dfs(idx, candidates,
 * target, acc, container): if target == 0: container.append(acc[::]) elif
 * target > 0: i = idx while i < len(candidates): acc.append(candidates[i])
 * dfs(i, candidates, target - candidates[i], acc, container) acc.pop() i += 1
 * while i < len(candidates) and candidates[i - 1] == candidates[i]: i += 1
 * else: pass
 * 
 * container = [] candidates = sorted(candidates) dfs(0, candidates, target, [],
 * container)
 * 
 * return container
 */
