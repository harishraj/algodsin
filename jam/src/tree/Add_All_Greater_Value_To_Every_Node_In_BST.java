...
public class Add_All_Greater_Value_To_Every_Node_In_BST {
	...
	/**
	 * Look at this question this way You have a sorted array in descending
	 * order and you want [sum(arr[:i]) for i in xrange(len(arr))]. So you
	 * definitely need an accumulater. And you want to do it in descending
	 * order. Reversed inorder traversal. Here we go.
	 */
	...
	private static void modifyBSTUtil(TreeNode root, intWrapper sum) {
		...
		modifyBSTUtil(root.right, sum); // Go right first. Descending inorder
										// traversal.
		sum.val += root.val;
		root.val = sum.val;
	...
	// This function is irrelevant to this question but itself is quite
	// interesting
