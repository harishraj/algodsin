...
class leetcode_Construct_Binary_Tree_from_Inorder_and_Postorder_Traversal {
	...
	public static TreeNode buildTree(int[] in, int[] post) {
		if (in.length != 0 && post.length != 0) {
			...
			int[] right_in = new int[in.length - idx - 1], right_post = new int[in.length
					- idx - 1];
...
/*
 * Python Version def buildTree(self, inorder, postorder): if not inorder and
 * not postorder: return None
 * 
 * val = postorder[-1] idx = inorder.index(val)
 * 
 * ret = TreeNode(val) ret.left = self.buildTree(inorder[:idx], postorder[:idx])
 * ret.right = self.buildTree(inorder[idx + 1:], postorder[idx : -1]) return ret
 */
