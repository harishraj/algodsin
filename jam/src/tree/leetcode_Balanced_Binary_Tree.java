...
public class leetcode_Balanced_Binary_Tree {
	...
	public static int maxD(TreeNode node) {
		if (node == null) {
			return 0;
		}
...
/*
 * Python Version class Solution: # @param root, a tree node # @return a boolean
 * def isBalanced(self, root): if not root: return False else: return
 * self.max_depth(root) != -1
 * 
 * def max_depth(self, root): if not root: return 0 left =
 * self.max_depth(root.left) if left < 0: return -1 right =
 * self.max_depth(root.right) if right < 0: return -1 if abs(left - right) > 1:
 * return -1 else: return max(left, right) + 1
 */
