...
public class google_Two_Sum {
	...
	// Using two stacks to simulate the inorder traversal with two pointers
	// solution.
	// Time complexity O(n).
	...
	public static void twoSum(TreeNode root, int target) {
		...
		// Using two stacks to store the left track of nodes and right track of
		// node.
		...
		while (leftStack.peek().val < rightStack.peek().val) {// exits when
																// root.val ==
																// root.val
			if (leftStack.peek().val + rightStack.peek().val > target) {
				...
				if (rightElem.left != null) {
					// Push the left child of cur node and all of its right
					// children to stack.
				...
				System.out.println(String.format("%d + %d = %d",
						leftStack.peek().val, rightStack.peek().val, target));
	...
	public static void two(TreeNode root, int target) {
		...
		while (head < tail) {
			if (arr.get(head).val + arr.get(tail).val < target) {
				...
				// Found.
				System.out.println(String.format("%d + %d = %d",
						arr.get(head).val, arr.get(tail).val, target));
				++head;
				--tail;
			}
		}
