...
public class leetcode_Unique_Binary_Search_Trees_II {
	...
	public static void nextNode(int[] nums, ArrayList<Integer> tmp,
			boolean[] used, HashSet<ArrayList<Integer>> set,
			ArrayList<TreeNode> all) {
 ...
 * Python Version
 * 
 * def generateTrees(self, n): def dfs(start, end): if start > end: return
 * [None]
 * 
 * if start == end: return [TreeNode(start)]
 * 
 * ret = [] for i in xrange(start, end + 1): leftNodes = dfs(start, i - 1)
 * rightNodes = dfs(i + 1, end)
 * 
 * 
 * for left in leftNodes: for right in rightNodes: node = TreeNode(i) node.left
 * = left node.right = right ret.append(node) return ret
 * 
 * return dfs(1, n)
 */
