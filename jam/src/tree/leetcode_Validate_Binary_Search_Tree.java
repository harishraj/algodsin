...
public class leetcode_Validate_Binary_Search_Tree {
	...
	public static boolean isValidBST(TreeNode root) {
		...
		// the array to check if value of every node is less than the latter
		// one.
...
/*
 * Python Version def isValidBST(self, root): def rec(node, up, lower): if not
 * node: return True if up > node.val and node.val > lower: return
 * rec(node.left, node.val, lower) and rec(node.right, up, node.val) else:
 * return False
 * 
