...
public class BST_Identical_Check_Without_Building_Trees {
	...
	public static boolean isIdenticalBSTUtil(int[] a, int[] b, int i1, int i2,
		...
		for (j = i1; j < a.length; ++j) {
			if (a[j] > min && a[j] < max) { // <= max depends on the definition
											// of BST.
				break;
			}
		}
		...
		// If the value of a is not equal to value of b then it's not idential
		// node
		// at the same position. Return false.
		// If Either of the element is last of the arrays, then current position
		// one tree
		...
		// The value of current node you just locate, is the uplimit of left
		// subtree
