...
public class cap_Suffix_Trie {
	...
	private static class SuffixTrieNode {
		...
		private final List<Integer> indexes; // Store the all indexes of a
												// specific char.
		...
		public void insert(String s, int index) {
			...
			if (s != null && s.length() > 0) {
				...
				if (child == null) {
					...
					kids.put(this.val, child); // Cache char/node pair if cur
												// char hasn't been cached.
				}
				// Recursion.
		...
		public List<Integer> getIndexes(String s) {
			if (s == null || s.length() == 0) {
				...
				if (this.kids.containsKey(first)) {
					...
					return Collections.emptyList(); // Non-existing suffix
													// string
				}
			}
		}
