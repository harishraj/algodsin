...
class linkedin_Binary_Search_Tree_Detele_Node {
	...
	public static void delete(int target, TreeNode root) {
		...
		if (cursor != null) {// cursor == null meaning after search the entire
								// tree but didn't find the target node.
			...
			if (cursor.left == null && cursor.right == null) {
				...
				if (isParentLeftKid) {
					...
					while (succesor.left != null) { // Trying to find succesor
													// of cursor node.
						succesorParent = succesor;
						succesor = succesor.left;
					}
					cursor.val = succesor.val; // Swap the val of succesor and
												// cursor.
					if (succesorParent == null) { // if succesor is cursor's
													// right kid
						cursor.right = succesor.right;
					} else { // Delete succesor and take care of succesor's
								// right kid.
