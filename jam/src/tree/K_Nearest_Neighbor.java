...
public class K_Nearest_Neighbor {
	...
	/*
	 * 1. max heap. O(nlogk). When k is small, time complexity is low. Space
	 * complexity O(k). 2. selection sort. O(n^2). Space Comparator O(1). 3. k-d
	 * tree(2-d tree in this case) (2-d tree is non-trivial...) 4. quick sort.
	 * O(nlogn)
	 */

	public static void main(String[] args) {
		// Test case.
		int[][] Points = new int[][] { { 3, 6 }, { 8, 99 }, { -1, 5 },
				{ 4, -9 }, { 0, 1 } };
	...
	public static ArrayList<Point> heap(int[][] pointSet, int[] origin, int k) {
		...
		for (int i = 0; i < pointSet.length; ++i) {
			queue.offer(new Point(pointSet[i][0], pointSet[i][1], origin[0],
					origin[1]));
		}
		for (int i = 0; i < k; ++i) {
			if (!queue.isEmpty()) {
	...
	// Selection sort.
	public static ArrayList<Point> selectionSort(int[][] pointSet,
			int[] origin, int k) {
		Point[] arr = new Point[pointSet.length];
		// Pre-processing.
		for (int i = 0; i < pointSet.length; ++i) {
			arr[i] = new Point(pointSet[i][0], pointSet[i][1], origin[0],
					origin[1]);
		}
	...
	// Quick sort.
	public static ArrayList<Point> quickSort(int[][] pointSet, int[] origin,
			int k) {
		Point[] arr = new Point[pointSet.length];
	...
	public static void quickSortUtil(Point[] arr, int k, int left, int right) {
		...
		if (leftLength == k) { // K smallest elements do not have to be in
								// order.
			return;
...
class Point implements Comparable<Point> {
	...
	public double eucDistSquare() {
		return Math.pow(this.x - this.originX, 2.0)
				+ Math.pow((this.y - this.originY), 2.0);
	}
