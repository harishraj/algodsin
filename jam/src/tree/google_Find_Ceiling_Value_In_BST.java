...
public class google_Find_Ceiling_Value_In_BST {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i <= 16; ++i) {
			System.out.println(String.format("Input: %d Output: %d", i,
					findCeilingValueInBST(root, i)));
		}
	}

	public static int findCeilingValueInBST(TreeNode root, int val) {
		// p, the value of current root's parent.
		// leftP, the value of parent where is the left child relationship along
		// the path.
		...
		while (root != null) {
			if (val < root.val) { // Go left.
				...
				if (root.right != null) {
					...
					// Tricky part.
					// 1. If val is larger than any values in the BST, return
					// MIN_VALUE
