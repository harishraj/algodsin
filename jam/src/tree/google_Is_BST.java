...
public class google_Is_BST {
	...
	public static boolean isBST(TreeNode root) {
		if (root == null) {
			return true; // Should check with your interviewer with this edge
							// case
		} else {
	...
	public static boolean isBSTUtil(TreeNode node, int max, int min) {
		if (node == null) {
			return true;
		} else if (node.val > max || node.val <= min) { // Shoudl check with
														// your interviewer
														// about the def of BST
			return false;
		} else {
			return isBSTUtil(node.left, node.val, min)
