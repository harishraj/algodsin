...
public class cap_First_Common_Ancestor {
	...
	/*
	 * 1. If each node has link pointing back to its parent, simply trace all
	 * the way back to their paths first meets. 2. Naive method. At each level
	 * of dfs, check if given nodes are on different sides.
	 */
	...
	// Robin's algo
	// 1. Assuming that each kid has a link pointing to its parent. Find the
	// depth of both nodes.
	// For the lower node, travel to its parent util two nodes are at the same
	// depth. Check if two
	// nodes are the same node. If yes, that the LCA. If no, both nodes travel
	// back to their parents
	// and check if they are the same. Rinse wash repeat util they are the same.
	// 2. Assuming that there is no link pointing to its parent. Then we need to
	// cache the path
	// from root to each node. The idea is all most the same as above. But space
	// complexity is
	...
	public static boolean getPathUtil(TreeNode root, TreeNode target,
	...
	public static TreeNode LCANaive(TreeNode root, TreeNode p, TreeNode q) {
		if (root == null) {
			return null;
				...
				&& (contains(root.left, q) || contains(root.right, q))) {
			return root;
