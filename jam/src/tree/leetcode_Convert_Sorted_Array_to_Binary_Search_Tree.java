...
public class leetcode_Convert_Sorted_Array_to_Binary_Search_Tree {
	...
	public static TreeNode nextNode(int[] num, int head, int tail) {
		if (head <= tail) {
			int middle = head + (tail - head) / 2; // avoid overflow (tail +
													// head).
...
/*
 * Python Version def sortedArrayToBST(self, num): def rec(head, tail, num): if
 * head > tail: return None
 * 
 * mid = head + (tail - head) / 2
 * 
 * cur = TreeNode(num[mid]) cur.left = rec(head, mid - 1, num) cur.right =
 * rec(mid + 1, tail, num) return cur
 * 
