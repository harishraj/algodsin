 ...
 * 
 * Usualy, bst is defined as: if an element is less than the value of current
 * node then it goes left. If it's larger than the value of current node then it
 * goes right.
 * 
 * Let's redefine it: if an element is equal or larger than the value of current
 * node then it goes right.
 * 
...
public class Find_Duplicates_In_BST {
	...
	public static boolean hasDuplicates(TreeNode root) {
		return (root == null) ? false : rec(root.left, root.val)
