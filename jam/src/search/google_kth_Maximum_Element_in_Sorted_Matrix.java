...
public class google_kth_Maximum_Element_in_Sorted_Matrix {
	...
	public static void main(String[] args) {
		...
		int[][] m = { { 1, 3, 5 }, { 2, 4, 8 }, { 6, 9, 10 } };
		for (int i = 1; i < 11; ++i) {
			System.out
					.println(String.format("%d: %d", i, getOrder(m, 3, 3, i)));
		}
	}
	...
	public static int search(int[][] m, int k) {
		...
		int mid = 0, order;
		// Binary search. Find a int mid, no matther it exists in matrix or not,
		// there are k
		...
		while (rowCursor < row && colCursor >= 0) {
			if (m[rowCursor][colCursor] < mid) {
				ret = (m[rowCursor][colCursor] > ret) ? m[rowCursor][colCursor]
						: ret;
				++rowCursor;
			} else {
				--colCursor;
	...
	// Disadvantage: Space O(M * N) to track the status of each element(added or
	// not).
	// Time O(n^2 logn)
	public static int kthMaximum(int[][] m, int k) {
		...
		while (k > 0) {
			...
			if (cor[0] > 0 && !dict.containsKey(m[cor[0] - 1][cor[1]])) {
				...
				dict.put(m[cor[0] - 1][cor[1]],
						new int[] { cor[0] - 1, cor[1] });
			}
			if (cor[1] > 0 && !dict.containsKey(m[cor[0]][cor[1] - 1])) {
				minHeap.offer(m[cor[0]][cor[1] - 1]);
				dict.put(m[cor[0]][cor[1] - 1],
						new int[] { cor[0], cor[1] - 1 });
			}
			--k;
		}
		return ret;
