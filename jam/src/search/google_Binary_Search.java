 ...
 * google
 * 
 * Miss target: return i, which A[i] > target and A[i - 1] < target. If
 * duplicate target exist, return i, where i is the max index among all
 * duplicates.
 */
	...
	public static int binarySearch(int[] A, int target, int head, int tail) {
		while (head <= tail) {
			...
			if (med > target) {
				...
				// med == target. Check if target exists between [med + 1,
				// tail].
