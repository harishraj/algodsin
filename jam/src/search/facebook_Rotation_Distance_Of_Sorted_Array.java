...
public class facebook_Rotation_Distance_Of_Sorted_Array {
	...
	public void solve() {
		...
		// Test case.
		System.out
				.println(rotateDistanceWithDups(new int[] { 2, 3, 4, 5, 6, 1 }) == 5);
		System.out
				.println(rotateDistanceWithDups(new int[] { 3, 4, 5, 6, 1, 2 }) == 4);
		System.out
				.println(rotateDistanceWithDups(new int[] { 4, 5, 6, 1, 2, 3 }) == 3);
		System.out
				.println(rotateDistanceWithDups(new int[] { 5, 6, 1, 2, 3, 4 }) == 2);
		System.out
				.println(rotateDistanceWithDups(new int[] { 6, 1, 2, 3, 4, 5 }) == 1);
		System.out
				.println(rotateDistanceWithDups(new int[] { 1, 2, 3, 4, 5, 6 }) == 0);

		System.out
				.println(rotateDistanceWithDups(new int[] { 1, 1, 1, 3, 1 }) == 4);
		System.out
				.println(rotateDistanceWithDups(new int[] { 1, 1, 3, 1, 1 }) == 3);
		System.out
				.println(rotateDistanceWithDups(new int[] { 1, 3, 1, 1, 1 }) == 2);
		System.out
				.println(rotateDistanceWithDups(new int[] { 3, 1, 1, 1, 1 }) == 1);
		System.out
				.println(rotateDistanceWithDups(new int[] { 1, 1, 1, 3 }) == 0);
	}

	/*
	 * The idea is very similar to
	 * leetcode_Search_in_Rotated_Sorted_Array_II.java Find the offset position
	 * that A[i] > A[i - 1] or A[i - 1] > A[i]. Watch out for indexOfBoundary
	 * error.
	 */
	public int rotateDistanceWithDups(int[] arr) {
		return (arr == null || arr.length == 0) ? -1 : bs(arr, 0,
				arr.length - 1);
	}
