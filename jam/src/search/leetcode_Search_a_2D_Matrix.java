...
public class leetcode_Search_a_2D_Matrix {
	...
	public static boolean searchMatrix(int[][] matrix, int target) {
		...
		while (start <= end) {
			...
			if (target < matrix[row][0]) {
				...
				// target is larger than first int and less than last int of
				// this row.
				break;
			}
		}
	...
	public static boolean searchMatrix1(int[][] matrix, int target) {
		return matrixBinarySearch(matrix, target, 0, matrix.length - 1, 0,
				matrix[0].length - 1);
	}

	public static boolean matrixBinarySearch(int[][] matrix, int target,
			int rHead, int rTail, int cHead, int cTail) {
		...
		if (rHead <= rTail && cHead <= cTail) {
			...
			if (middle == target) {
				...
				return matrixBinarySearch(matrix, target, rHead, rMid,
						cMid + 1, cTail)
						|| matrixBinarySearch(matrix, target, rMid + 1, rTail,
								cHead, cTail);
			} else {
				// middle > target. Check three submatrix.
				return matrixBinarySearch(matrix, target, rHead, rMid - 1,
						cHead, cTail)
						|| matrixBinarySearch(matrix, target, rMid, rTail,
								cHead, cMid - 1);
			}
		}
		return false;
...
/*
 * Python Version def searchMatrix(self, matrix, target): x = 0 y =
 * len(matrix[0]) - 1 while x < len(matrix) and y >= 0: if matrix[x][y] ==
 * target: return True elif matrix[x][y] > target: y -= 1 else: x += 1
 * 
 * return False
 */
