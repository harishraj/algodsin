...
public class Find_Number_Of_Larger_Element_On_Right_Side {
	 ...
	 * 
	 * 1 3 5 2 4 6 -->> 1 2 3 4 5 6 2 1 0 2 1 0 5 4 3 2 1 0
	 * 
	 * 
	 */
	private Item[] merge(Item[] left, Item[] right) {
		...
		while (i < left.length && j < right.length) {
			if (left[i].num < right[j].num) {
				ret[i + j] = new Item(left[i].num, left[i].count
						+ right[j].count + 1);
				i += 1;
	 ...
	 * elements from right to left. It's O(lgn) to insert and return the
	 * inserted node.
	 * 
	...
	private static class TreeNode {
		 ...
		 * Insert into BST.
		 * 
		 * Incre the count of the visited node by one. By maintaining this value
		 * to ge the total # of nodes in its subtree by O(1).
		 */
