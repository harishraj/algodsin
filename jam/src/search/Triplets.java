...
public class Triplets {
	 ...
	 * 
	 * Iterate from left to right as the first element in triplet. Then pick the
	 * second element right next to the first one. And now you have two fixed
	 * element in a triplet and given n, you now know the range of the third
	 * element in a triplet, which should be within range (second, n - first -
	 * second). Binary search will help you find the index and the range.
	 * 
	 * Now the move the second element one step right and then do *NOT* use
	 * binary search again, simply move the third element one step right until
	 * you find the one that meets the requirement and the range.
	 * 
	...
	public void solve(int[] arr, int n) {
		...
		for (int i = 0; i < arr.length - 2; ++i) {
			int j = i + 1;
			int index = binarySearch(arr, n - arr[i] - arr[j], j + 1,
					arr.length - 1);
	 ...
	 * 
	 * Return its index. Option one: implement binary search. This method itself
	 * can be a interview question itself. Time complexity O(lgn)
	 * 
