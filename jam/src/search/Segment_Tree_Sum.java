...
public class Segment_Tree_Sum {
	...
	public static void main(String[] args) {
		System.out.println(segmentTree(
				new int[] { 2, 4, 3, 1, 6, 7, 8, 9, 1, 7 }, 2, 7));
	...
	public static int segmentTree(int[] A, int start, int end) {
		...
		// For a non-leaf node numbered x, the left son of x is 2*x and the
		// right son 2*x+1.
		// Height of segment tree: (int)(Math.ceil(Math.log(A.length) /
		// Math.log(2)))
	...
	private static void updateSTreeUtil(int[] STree, int start, int end, int i,
			int diff, int index) {
		if (i < start || i > end) {
			return;
		}
	...
	private static int rangeSumUtil(int[] STree, int start, int end,
			int qStart, int qEnd, int index) {
		if (qStart <= start && qEnd >= end) {
			return STree[index]; // The query range is larger than range current
									// node represents.
			...
			return rangeSumUtil(STree, start, mid, qStart, qEnd, 2 * index + 1)
					+ rangeSumUtil(STree, mid + 1, end, qStart, qEnd,
							2 * index + 2);
		}
	}
	...
	private static int buildSTreeUtil(int[] arr, int start, int end,
			int[] STree, int index) {
		if (start == end) {
