...
public class Find_A_Element_Appears_No_Less_Than_One_Forth_Times_In_Sorted_Array {
	...
	public static void main(String[] args) {
		Find_A_Element_Appears_No_Less_Than_One_Forth_Times_In_Sorted_Array solution = new Find_A_Element_Appears_No_Less_Than_One_Forth_Times_In_Sorted_Array();

		System.out.println(solution.solve(new int[] { 1, 2, 3, 4, 4, 4, 5 }));
	}

	/**
	 * First find the range of element in the middle, if it meets the
	 * requirement, then return.
	 * 
	 * If not, the array has been divided into three parts, binary search the
	 * left and right parts. The tricky part is for the one-level deeper
	 * recursion, if still hit, then there is not hit at all for that part. That
	 * is because in second recursion, the divided parts are less than length
	 * 4/n, no need to keep recursing.
	 * 
	...
	private int solveUtil(int[] arr, int recLvl) {
		...
		int leftPartSearchRes = solveUtil(
				Arrays.copyOfRange(arr, 0, range.start - 1), 1);
		if (leftPartSearchRes != -1) {
			return leftPartSearchRes;
		}

		int rightPartSearchRes = solveUtil(
				Arrays.copyOfRange(arr, range.end + 1, arr.length - 1), 1);
	...
	/**
	 * TODO: How to use interface to reduce the duplicate code of binarySearch
	 * for left/right boundary?
	 */
