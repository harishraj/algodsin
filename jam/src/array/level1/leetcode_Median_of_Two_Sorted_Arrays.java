...
class leetcode_Median_of_Two_Sorted_Arrays {
	...
	public static void main(String[] args) {
		System.out.println(findMedianSortedArrays1(new int[] { 4 }, new int[] {
				1, 2, 3 }));
	 ...
	 * concept since median brings even/odd length problem. Simply finding the
	 * kth largest element will help write clean code.
	 * 
	 */
	...
	public static double find(int[] A, int n, int[] B, int m, int th) {
		if (n == 0) {
			return B[th];
		}
		if (m == 0) {
		...
		if (midA + 1 + midB + 1 <= th + 1) {
			...
			// (b0..b3) <= (a2) <= (a3 a4) is know. Where (b4b5b6) can be? Can
			// be anywhere between(b3, a2..a4)
			// Can not dump any a because when b4b5b6 are between b3 and a2 when
			// kth will be in a2-a4
			// Conclusion: It's only safe to dump b0b1b2b3.
			return find(A, n, Arrays.copyOfRange(B, midB + 1, m), m
					- (midB + 1), th + 1 - (midB + 1) - 1);
		} else {
	...
	public static double medianSortedArrays(int[] A, int[] B) {
		...
		if (n == 0) {
			...
			if (m == 1) {
				...
				return (B[m / 2] + MedianOfThree(A[0], B[m / 2 - 1],
						B[m / 2 + 1])) / 2.0;
			} else { // m is even.
			...
			if (m % 2 == 1) { // m is odd.
				return MedianOfThree(B[m / 2], Math.min(A[0], B[m / 2 + 1]),
						Math.max(A[1], B[m / 2 - 1]));
			} else { // m is even.
				return MedianOfFour(B[m / 2 - 1], B[m / 2],
						Math.min(A[0], B[m / 2 + 1]),
						Math.max(A[1], B[m / 2 - 2]));
			}
		}
		...
		if (A[idxA] < B[idxB]) {
			...
			return medianSortedArrays(
					Arrays.copyOfRange(A, minRemoved, A.length),
					Arrays.copyOfRange(B, 0, B.length - minRemoved));
		} else {
			if (m % 2 == 0) {
				--idxB;
			}
			...
			return medianSortedArrays(
					Arrays.copyOfRange(A, 0, A.length - minRemoved),
					Arrays.copyOfRange(B, minRemoved, B.length));
		}
	}
}

/*
 * Python version http://blog.csdn.net/sigh1988/article/details/12192299 Time
 * complexity O(lgm + lgn)
 * 
 * def findMedianSortedArrays(self, A, B): m = len(A) n = len(B) mid = (m + n) /
 * 2 if (m + n) % 2 == 1: return self.find(A, m, B, n, mid) else: left = right -
 * 1 return (self.find(A, m, B, n, mid - 1) + self.find(A, m, B, n, mid)) / 2.0
 * 
 * def find(self, A, m, B, n, th): if m == 0: return B[th] if n == 0: return
 * A[th]
 * 
 * mida = (m - 1) / 2 midb = (n - 1) / 2 if A[mida] < B[midb]: return
 * self.find(B, n, A, m, th) # else A[mida] >= B[mida] if mida + 1 + midb + 1 <=
 * th + 1: return self.find(A, m, B[midb + 1:], n - (midb + 1), (th + 1) - (midb
 * + 1) - 1) else: return self.find(A, mida, B, n, th)
 */
