...
class leetcode_Longest_Consecutive_Sequence {
	public static void main(String[] args) {
		...
		System.out.println(longestConsecutive(new int[] { -6, 6, -9, -7, 0, 3,
				4, -2, 2, -1, 9, -9, 5, -3, 6, 1, 5, -1, -2, 9, -9, -4, -6, -5,
				6, -1, 3 }));
	}
	...
	public static int longestConsecutive(int[] num) {
		...
		for (int v : num) {
			// Remove by sequence. Find the longest sequence containing point v
			// and remove them
			// after recording its length.
			if (set.contains(v)) {
				ans = Math.max(ans,
						getCount(set, v, false) + getCount(set, v + 1, true));
			}
		}
		return ans;
	}
	...
	public static int longestConsecutive1(int[] num) {
		// You can sort the array first but it's gonna take nlogn. We can do
		// better than that.
		// The idea behind this is using int[2] to represent the interval.
		// Loop over the given int array and for each integer i, check if i+1
		// and i-1 are in the table.
		final int leftEnd = 0;
		...
		for (int i : num) {
			...
			if (largerInterval == null && smallerInterval == null) {
				...
				largeEnd[leftEnd] = smallEnd[leftEnd]; // Extend larger's
														// leftEnd to small's
														// leftEnd.
				smallEnd[rightEnd] = largeEnd[rightEnd]; // Extend small's
															// rightEnd to
															// large's rigthEnd.
				map.put(largerInterval[rightEnd], largeEnd);
				map.put(smallerInterval[leftEnd], smallEnd);
				map.put(i, smallEnd);
				maxLength = Math.max(maxLength, largeEnd[rightEnd]
						- largeEnd[leftEnd] + 1);
			} else {
				if (largerInterval != null) { // new small edge found.
					int[] largeEnd = map.get(largerInterval[rightEnd]);
					largeEnd[leftEnd] = i;
					largerInterval[leftEnd] = i; // updating interval's small
													// end.
					map.put(largerInterval[rightEnd], largeEnd);
					...
					maxLength = Math.max(maxLength, smallEnd[rightEnd]
							- smallEnd[leftEnd] + 1);
				}
			}
		}
 ...
 * Python Version
 * 
 * def longestConsecutive(self, num): s = set(num) ret = 0 for i in num: if not
 * i in s: continue
 * 
 * up = i while up + 1in s: s.remove(up + 1) up += 1 down = i while down - 1 in
 * s: s.remove(down - 1) down -= 1
 * 
 * ret = max(ret, up - down + 1) return ret
 */
