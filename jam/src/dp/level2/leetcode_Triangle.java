		...
		for (int i = 1; i < triangle.size(); ++i) {
			...
			for (int j = 0; j < layer.size(); ++j) {
				if (j == 0) {
					...
					int minCost = Math.min(prevLayerCost.get(j - 1),
							prevLayerCost.get(j));
...
/*
 * Python Version def minimumTotal(self, triangle): n = len(triangle) dp = [ [ 0
 * for _ in xrange(n) ] for _ in xrange(n) ] dp[0][0] = triangle[0][0] for i in
 * xrange(1, n): for j in xrange(i + 1): if j == 0: prev_min = dp[i - 1][j] elif
 * j == i: prev_min = dp[i - 1][j - 1] else: prev_min = min(dp[i - 1][j - 1],
 * dp[i - 1][j]) dp[i][j] = prev_min + triangle[i][j]
 * 
 * return min(dp[-1])
 */
