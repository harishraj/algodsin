...
class Robot_Moving {
	public static void main(String args[]) {
		...
		myPath(new int[] { 0, 0, 1, 0, 2, 3, 1, 3, 2, 0, 3 }, new int[] { 0, 1,
				2, 2, 1, 2, 2, 1, 2, 2, 1 });
		my(new int[] { 0, 0, 1, 0, 2, 3, 1, 3, 2, 0, 3 }, new int[] { 0, 1, 2,
				2, 1, 2, 2, 1, 2, 2, 1 });
	}
	...
	public static void myPath(int[] two, int[] one) {
		...
		while (index < one.length) {
			cost[index] = Math.min(cost[index - 1] + one[index],
					cost[index - 2] + two[index]);
			++index;
		}
		index = cost.length - 1;
		path.add(index);
	...
	public static int nextCost(int[] two, int[] one, int x) {
		...
		return Math.min(nextCost(two, one, x - 1) + one[x],
				nextCost(two, one, x - 2) + two[x]);
	}
	...
	public static void my(int[] two, int[] one) {
		...
		while (index < one.length) {
			int tmp = Math
					.min(one_before + one[index], two_before + two[index]);
