...
class google_Maximum_Money_Collected_By_Two_People {
	public static void main(String[] args) {
		int[][] matrix = { { 40, 40, 0 }, { 50, 50, 50 }, { 0, 40, 40 } };
		// System.out.println(maxMoneyOnePeople(matrix));
		System.out.println(maxMoneyTwoPeople(matrix));
	}
			...
			for (int j = 0; j < n; ++j) {
				for (int h = m - 1; h >= 0; --h) {
					for (int k = 0; k < n; ++k) {
						...
						// track[i][i - 1][h][k],
						// track[i][j][h + 1][k],
						// track[i][j][h][k - 1])) Be careful with
						// indexOfBoundary
						// + matrix[i][j] if (i == h && j == k) Money can be
						// collected by only one guy
						// matrix[i][j] + matrix[h][k] else
						int leftLeft = (j != 0 && k != 0) ? track[i][j - 1][h][k - 1]
								: 0;
						int leftDown = (j != 0 && h != m - 1) ? track[i][j - 1][h + 1][k]
								: 0;
						int downLeft = (i != m - 1 && k != 0) ? track[i + 1][j][h][k - 1]
								: 0;
						int downDown = (i != m - 1 && h != m - 1) ? track[i + 1][j][h + 1][k]
								: 0;
						track[i][j][h][k] = maxOfFour(leftLeft, leftDown,
								downLeft, downDown);
						if (i == h && j == k) {
