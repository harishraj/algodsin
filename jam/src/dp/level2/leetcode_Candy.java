...
public class leetcode_Candy {
	...
	public static void candies(int[] arr, int n) {
		...
		// The idea behind this is using two array to track increasing array and
		// decreasing array seperately.
 ...
 * Python Version
 * 
 * def candy(self, ratings): left_to_right = [ 1 for _ in xrange(len(ratings)) ]
 * for i in xrange(len(ratings) - 1): if ratings[i] < ratings[i + 1]:
 * left_to_right[i + 1] = left_to_right[i] + 1
 * 
 * right_to_left = [ 1 for _ in xrange(len(ratings)) ] for i in
 * reversed(xrange(1, len(ratings))): if ratings[i - 1] > ratings[i]:
 * right_to_left[i - 1] = right_to_left[i] + 1
 * 
 * return sum([ max(left_to_right[i], right_to_left[i]) for i in
 * xrange(len(ratings)) ])
 */
