...
public class google_Domino_Tiling {
	 ...
	 * here j in binary form by (0 0 meaning a horizontal tile and 1 a vertical
	 * one) 0 And we want the last row to be 0 0...0 0 (m x 1's), which demical
	 * value is 0. dp[height - 1][0] is the final result.
	 */
	public static void solve(int height, int width) {
		...
		// 1 << width = 2^width, represents how many different ways to tile 1 x
		// width.
	...
	private static void calc(int i, int j, int status, long[][] dp, int height,
			int width) {
		// System.out.println(String.format("i: %d j: %d status: %s", i, j,
		...
		if (j > width) {
			...
			if (i == 0) { // First row.
				...
				for (int k = 0; k < dp[i].length; ++k) {
					// (status ^ k) == status - k, what does this mean? For all
					// different bits, bits in status are all 1's and bits in k
					// are all 0's (no borrow during substraction)
					// meaning current row got 1 (vertical) then previous MUST
					// be 1.
					// But using two 1's will be confusing so use 0 to mark as
					// closure of vertical tile.
					// status is a valid piling for row i. and k is all possible
					// piling for a row.
					if ((status ^ k) == status - k) {
						dp[i][status ^ k] += dp[i - 1][k]; // status ^ k, 1111
															// and 1100 -> 0011
															// 1100
															// 0011
					}
			...
			calc(i, j + 1, (status << 1) + 1, dp, height, width); // left shit
																	// by 1 and
																	// add one
																	// meaning
																	// vertical
																	// pile.
			calc(i, j + 2, (status << 2), dp, height, width); // left shift by 2
																// meaning put
																// two zeros at
																// the end of
																// status
	...
	public static void dfs(int height, int width) {
		...
		System.out.println(String.format("height:%d Width:%d num:%s", height,
				width, all.size()));
	}

	public static void dfsUtil(int i, int j, int height, int width,
			int[][] domino, ArrayList<int[][]> all) {
		if (j == width) {
			// New row.
