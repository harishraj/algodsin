...
class Longest_Palindrome_Substring {
	...
	public static String manacher(String A) {
		...
		int pivot = 0; // pivot of the longest palindrome substring found so
						// far.
		int rightEdge = 0; // right edge of longest palindrome substring found
							// so far.
		for (int i = 1; i < n - 1; ++i) {
			// pivot = (i + mirror) / 2. Find the symmetry point of i against on
			// pivot.
			...
			// 2. rightEdge <= i, the range of current longest palindrome
			// substring outreaches i. Symmetric info can no be reused. set
			// statuc[i] = 0.
			status[i] = (rightEdge > i) ? Math.min(rightEdge - i,
					status[mirror]) : 0;
		...
		return A.substring((longestPivot - longestRadius) / 2,
				(longestPivot + longestRadius) / 2);
	}
}

/*
 * Python Version (TLE!?) def longestPalindrome(self, s): if not s or len(s) ==
 * 1: return s
 * 
 * ret = '' local_max = 1
 * 
 * dp = [ [ False for _ in xrange(len(s)) ] for _ in xrange(len(s)) ] # Init for
 * i in xrange(len(s)): dp[i][i] = True
 * 
 * for i in reversed(xrange(len(s) - 1)): for j in xrange(i + 1, len(s)):
 * dp[i][j] = (s[i] == s[j]) and (j == i + 1 or dp[i + 1][j - 1]) if dp[i][j]
 * and j - i + 1 > local_max: ret = s[i : j + 1] local_max = j - i + 1
 * 
 * return ret
 */
