...
public class leetcode_Distinct_Subsequences {
	...
	public static int num(String S, String T) {
		// This way we need two dp state array to track but for loop logic is
		// more clear.
...
/*
 * Python VersionNote* Make sure you got the dp relation right before code it
 * out.
 * 
 * def nmDistinct(self, S, T): dp = [ 1 ] * (len(S) + 1)
 * 
 * # Rest of the dp status. for i in xrange(1, len(T) + 1): nex = [ 0 ] *
 * (len(S) + 1) for j in xrange(i, len(S) + 1): nex[j] = dp[j - 1] + nex[j - 1]
 * if T[i - 1] == S[j - 1] else nex[j - 1] dp = nex
 * 
 * return dp[-1]
 */
