...
public class cap_Maximum_Submatrix {
	 ...
	 * Dynamic programming.
	 * 
	 * Brutal force: O(n^6). O(n^4) to traversal all possible submatrix and
	 * O(n^2) to compute the sum
	 * 
	 * O(n^3) as follow
	 */
		...
		for (int i = 0; i < n; ++i) {
			...
			for (int j = i; j < n; ++j) {
				...
				for (int it = 0; it < m; ++it) {
					accumulator[it] += mtx[j][it]; // accumulator the value
													// along y axis.
				}
