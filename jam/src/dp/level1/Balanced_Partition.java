...
class Balanced_Partition {
	...
	public static void partition(int[] arr) {
		...
		for (i = 1; i <= sum / 2; ++i) {
			status[i][0] = arr[0] == i; // if a0 can sum up to i only when a0 =
										// i.
			for (int j = 1; j < arr.length; ++j) {
				// Check if (a, a1..aj-1) can sum up to i - j (i - j >= 0).
				boolean oneStepBack = (i - arr[j] >= 0) ? status[i - arr[j]][j - 1]
						: false;
				// if (a0,a1..aj-1) can sum up to i, then (a0,a1..aj) can sum up
				// to i too.
