...
class Matrix_Chain_Operation {
	...
	public static int solve(int[] dim) {
		...
		for (L = 2; L < n; ++L) { // length of span.
			for (i = 1; i < n - L + 1; ++i) { // row 0 will not be used to make
												// index easily understood.
				j = i + L - 1; // index of last in cur span.
				dp[i][j] = Integer.MAX_VALUE; // Set default to max for further
												// min operation.
				for (k = i; k <= j - 1; ++k) {
					// dp relationship:
					// dp[i][j] = min(dp[i][k] + dp[k + 1][j] + dim[i - 1] *
					// dim[k] * dim[j]) for all k in [i, j-1]
					// @param dp[i][j]: min calculation operation from matrix i
					// to matrix j.
					// dim[i - 1] * dim[k] * dim[j]: the operation needed
					// formatrix [i, k] multiplied by matrix[k + 1][j]
