...
class leetcode_Interleaving_String {
	...
	/*
	 * DP based. s3 start with either s1[0] or s2[0]. Otherwise return false.
	 * 
	 * if s3 start with s1[0], subproblem: s1[1:], s2, s[1:] if s3 start with
	 * s2[0], subproblem: s, s2[1:], s[1:] (if s1[0] == s2[0], take or
	 * operation)
	 * 
	 * if s1[:i-1] and s2[:j-1] is interleaving word, then dp[i][j] == true else
	 * false.
	 */
	...
	public boolean util(String s1, String s2, String s3) {
		if (s1.length() + s2.length() != s3.length()) {
			...
			return util(s1.substring(1), s2, s3.substring(1))
					|| util(s1, s2.substring(1), s3.substring(1));
...
/*
 * Python Version def isInterleave(self, s1, s2, s3): if len(s1) + len(s2) !=
 * len(s3): return False
 * 
 * dp = [ False for _ in xrange(len(s2) + 1) ] dp[0] = True for i in
 * xrange(len(s2)): if s2[i] == s3[i]: dp[i + 1] = dp[i]
 * 
 * for i in xrange(len(s1)): nex = [ False for _ in xrange(len(s2) + 1) ] nex[0]
 * = s1[:i + 1] == s3[:i + 1] for j in xrange(len(s2)): use_s2 = nex[j] if s2[j]
 * == s3[i + j + 1] else False use_s3 = dp[j + 1] if s1[i] == s3[i + j + 1] else
 * False nex[j + 1] = use_s2 or use_s3 dp = nex
 * 
 * return dp[-1]
 */
