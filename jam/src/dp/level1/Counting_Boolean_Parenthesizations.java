...
class Boolean_Expression_Parenthesization {
	...
	public static int parenthesize(String A) {
		...
		// Init.
		for (int i = 0; i < n; ++i) {
			s[i][i] = (arr[2 * i].equals("true")) ? new Element(1, 0)
					: new Element(0, 1); // Based case: s[i][i].
			for (int j = i + 1; j < n; ++j) {
		...
		for (int i = n - 2; i >= 0; --i) {
			for (int j = i + 1; j < n; ++j) {
				for (int k = i; k < j; ++k) {
					/*
					 * s(i, k) op s(k, j) "and" # of True = (i, k).True * (k,
					 * j).True # of False = (i,j).True * (k, j).False + (i,
					 * j).False * (k, j).Ture + (i, j).False * (k, j).False;
					 * "or" # of True = Total(i, j) * Total(k, j) - # of False #
					 * of False = s(i, j).False * s(j, k).False "xor" # of True
					 * = s(i, j).False * s(j, k).True + s(i, j).True * s(j,
					 * k).False # of False = s(i, j).True * s(j, k).True + s(i,
					 * j).False * s(j, k).False
					 */
					if (arr[2 * k + 1].equals("and")) {
						s[i][j].True += s[i][k].True * s[k + 1][j].True;
						s[i][j].False += s[i][k].True * s[k + 1][j].False
								+ s[i][k].False * s[k + 1][j].True
								+ s[i][k].False * s[k + 1][j].False;
					} else if (arr[2 * k + 1].equals("or")) {
						s[i][j].True += s[i][k].True * s[k + 1][j].False
								+ s[i][k].False * s[k + 1][j].True
								+ s[i][k].True * s[k + 1][j].True;
