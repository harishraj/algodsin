...
class leetcode_Palindrome_Partitioning_II {
	...
	public static int toPalindrome1(String str) {
		...
		for (i = 1; i < n; ++i) {
			if (dp[0][i]) {
				status[i] = 0; // is str.substring(0,i+1) is palindrome, needs 0
								// cut.
			} else {
				status[i] = status[i - 1] + 1; // cut init.
				for (j = 1; j < i; ++j) {
					status[i] = (dp[j][i]) ? Math.min(status[i],
							status[j - 1] + 1) : status[i];
				}
			}
		}
		return status[n - 1];
	...
	public static int[] allPalindrome(String A) {
		...
		for (int i = 1; i < n - 1; ++i) {
			int mirror = 2 * pivot - i; // pivot = (i + mirror) / 2. Find the
										// symmetry point of i based on pivot.
...
/*
 * Python Version def minCut(self, s):
 * 
 * n = len(s) dp = [ [ True if i == j else False for i in xrange(n) ] for j in
 * xrange(n) ]
 * 
 * for i in reversed(xrange(n - 1)): for j in xrange(i + 1, n): dp[i][j] = (s[i]
 * == s[j]) and (i + 1 == j or dp[i + 1][j - 1])
 * 
 * cut = [ 0 for _ in xrange(n) ]
 * 
 * for i in xrange(1, n): if dp[0][i]: cut[i] = 0 else: cut[i] = cut[i - 1] + 1
 * for j in xrange(1, i): if dp[j][i]: cut[i] = min(cut[i], cut[j - 1] + 1)
 * return cut[-1]
 */
