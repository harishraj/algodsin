...
class Longest_Increasing_Subsequence {
	public static void main(String[] args) {
		System.out.println(longestSubseq(new int[] { 0, 8, 4, 12, 2, 10, 6, 14,
				1, 9, 5, 13, 3, 11, 7, 15 }));
	}

	public static int longestSubseq(int[] seq) {
		// dp-based
		// Convert the questions to: longest increasing subsequence ends with
		// cur number.
		...
		int res = 0;
		for (int i : status) { // Iterator to find the longest increasing
								// subseq.
			res = Math.max(res, i);
		}
		return res;
