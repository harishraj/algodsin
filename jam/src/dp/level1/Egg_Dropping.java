...
public class Egg_Dropping {
	...
	public static int solve(int k, int n) {
		...
		// if an egg breaks on floor x, then you have k - 1 eggs for floor 1 to
		// floor x - 1
		// if an egg does not break on floor x, then you have k eggs for floor x
		// + 1 to floor n.
		if (k == 1) {
			// When you have only one egg left, you have to try one floor by on
			// floor.
			return n;
		} else if (n == 1 || n == 0) {
		...
		for (i = 1; i <= n; ++i) {
			res = Math.max(solve(k - 1, i - 1), solve(k, n - i)); // max: assume
																	// worst
																	// case.
