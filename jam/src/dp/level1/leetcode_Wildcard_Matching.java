...
class leetcode_Wildcard_Matching {
			...
			if (S == P || P == '?') {
				...
				if (P == '*') {
					...
					while (pp < p.length() && p.charAt(pp) == '*') {
						// Star match any sequence of character. Mutliple stars
						// is same as one start.
						++pp;
					}
					if (pp == p.length()) {
						// If p has one or many stars at the end, it will match
						// what's left in s.
						return true;
					}
	...
	public static boolean isMatch(String s, String p) {
		// The idea behind this is basically same as
		...
		for (int i = 0; i < s.length(); ++i) {
			...
			for (int j = 0; j < p.length(); ++j) {
				...
				if (charS == charP || charP == '?') {
					...
					// Mismatch. This else is not necessary because the default
					// is false.
					next[j + 1] = false;
				}
			}
