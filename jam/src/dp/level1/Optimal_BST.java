	...
	public static void solve(int[] node, int[] freq) {
		...
		for (L = 2; L <= n; ++L) { // L is the chain length.
			for (i = 0; i <= n - L; ++i) { // row index
				...
				for (r = i; r <= j; ++r) { // make a BST out of arr[i:j] with
											// root arr[r].
					// if r == i, left child is empty and right child is dp[i][r
					// - 1]
					// if r == j, right child is empty and left child is dp[r +
					// 1][j]
					int c = ((r > i) ? dp[i][r - 1] : 0)
							+ ((r < j) ? dp[r + 1][j] : 0) + sum(arr, i, j);
