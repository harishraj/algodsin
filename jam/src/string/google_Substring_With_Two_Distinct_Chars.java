...
public class google_Substring_With_Two_Distinct_Chars {
	...
	public static String solve(String str) {
		...
		while (tail < str.length()) {
			if (set.contains(str.charAt(tail))) {
				...
				// update ret.
				ret = (tail - start > ret.length()) ? str
						.substring(start, tail) : ret;
		...
		if (set.size() != 2) { // For edge case like "a"
			...
			return (tail - start > ret.length()) ? str.substring(start, tail)
					: ret;
		}
	}
}
...
/*
 * Python Version class Solution(object):
 * 
 * def solve(self, s): head = tail = start = 0 used = set() ret = '' while tail
 * < len(s): if s[tail] in used: tail += 1 elif len(used) < 2: used.add(s[tail])
 * head = tail tail += 1 else: ret = s[start : tail] if tail - start > len(ret)
 * else ret
 * 
 ...
 * used.clear()
 * 
 * # Wrap up what is left in used. if len(used) < 2: # The whole string contains
 * only one char return '' else: return s[start : tail] if tail - start >
 * len(ret) else ret
 * 
 * def run(self): print self.solve('abcbcbcbcbcddd') print
 * self.solve('abbbcccbbbcccd')
 * 
 * 
