...
public class google_Most_Common_Words {
	...
	public static void main(String[] args) {
		...
		Collections.addAll(input, "This is a darn good day",
				"This was a darn bad day", "That was darn good day",
	...
	public static List<String> mostCommonWords(List<String> sentences) {
		...
		for (int i = 0; i < sentences.size(); ++i) {
			...
			// similarities[i] represent the similarities between cur sentence
			// and ith
			// sentences.
			int[] similarities = new int[sentences.size()];
			Arrays.fill(similarities, 0);

			int curMaxSimilarity = 0; // max similarity between cur sentence and
										// others.
			int curMaxId = -1; // the id of sentence which has the max
								// similarity with cur sentence.
			...
			for (String word : sentence.split("\\s+")) {
				if (dict.containsKey(word)) {
					...
					// Creating index. It's very much alike search engine
					// indexing.
		...
		if (similarity != 0) {
			...
			Collections.addAll(retval, sentences.get(maxId1),
					sentences.get(maxId2));
			return retval;
		} else {
