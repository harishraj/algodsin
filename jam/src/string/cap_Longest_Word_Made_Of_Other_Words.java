...
class cap_Longest_Word_Made_Of_Other_Words {
	public static void main(String[] args) {
		System.out.println(findLongestWord(new String[] { "wohaojimo", "stoop",
				"shit" }, new String[] { "sto", "op", "shit", "dump", "oo" }));
	}
	...
	public static boolean isMadeOfOtherWords(String word, HashSet<String> set) {
		if (word == null || word.length() == 0) {
			...
			for (int i = 1; i < word.length(); ++i) {
				if (isMadeOfOtherWords(word.substring(0, i), set)
						&& isMadeOfOtherWords(word.substring(i), set)) {
					return true;
				}
			}
			return false;
