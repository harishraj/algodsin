...
class KMP {
	...
	public static int kmp(String A, String target) {
		...
		for (int i = 1; i < target.length(); ++i) {
			...
			int index = overlay[i - 1]; // Fetching index of previous
										// substring's pattern.
			while (index >= 0 && target.charAt(i) != target.charAt(index + 1)) {
				// index == -1: End loop. For current substring, no previous
				// found pattern
				// target.charAt(i) != target.charAt(index + 1): there is prev
				// pattern
				...
				// we know that for substring.(0, i - 1), first index substring
				// equals to
