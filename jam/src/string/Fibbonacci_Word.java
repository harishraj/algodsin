...
public class Fibbonacci_Word {
	public static void main(String[] args) {
		...
		// for (int n = 0; n < 30; ++n) {
		// for (int k = 1; k < 10; ++k) {
		// System.out.println("n: " + n + " k: " + k + " brutal force: " +
		// brutalForce(n, k) + " else: " + fibbonacciWordCount(n, k));
		// }
		// }
	}
	...
	public static int fibbonacciWordCount(int n, int k) {
		...
		String oddPattern = "";
		String evenPattern = "";
		int[] count = new int[3]; // count[0]: base case count; count[1]:
									// oddPattern count; count[2]: evenPattern
									// count;
		// Find the i Fibbonaci word W(i) that can not break into long enough
		// W(i - 1) & w(i - 2)
		while (true) {
		...
		// Find repeating patterns of concatenation part when n is odd and n is
		// even.
		if (base % 2 == 0) {
			oddPattern = baseCase.substring(baseCase.length() - shift,
					baseCase.length())
					+ baseNext.substring(0, shift);
			evenPattern = baseNext.substring(baseNext.length() - shift,
					baseNext.length()) + baseCase.substring(0, shift);
		} else {
			oddPattern = baseNext.substring(baseNext.length() - shift,
					baseNext.length())
					+ baseCase.substring(0, shift);
			evenPattern = baseCase.substring(baseCase.length() - shift,
					baseCase.length()) + baseNext.substring(0, shift);
		}
		if (k == 1) {
			base = 1; // ad-hoc..
		}
		...
		// System.out.println("base case index " + base + " " + baseCase +
		// " pattern " + oddPattern + " " + evenPattern);
	...
	public static void sumFreq(HashMap<String, Integer> global,
			HashMap<String, Integer> target, int freq) {
	...
	public static HashMap<String, Integer> substringFreq(String A, int k) {
		// Given string A and length k, return the frequency of substring with
		// length k.
	...
	public static void fibbonacciFreq(int n, int base, int[] count) {
		// Fibbonaci recursion. Counting the appearance of base cases, odd cases
		// and even cases.
		if (n < base) {
			return;
		}
