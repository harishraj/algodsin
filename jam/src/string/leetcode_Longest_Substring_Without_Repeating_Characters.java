...
/*
 * Python version. def lengthOfLongestSubstring(self, s): ret = 0
 * 
 * head = 0 tail = 0 dic = {}
 * 
 * while tail < len(s): if s[tail] in dic: ret = max(ret, tail - head) head =
 * tail = dic[s[tail]] + 1 dic.clear() else: dic[s[tail]] = tail tail += 1
 * 
