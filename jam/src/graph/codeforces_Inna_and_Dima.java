...
public class codeforces_Inna_and_Dima {
	...
	private final int[][] delta = new int[][] { { -1, 0 }, { 1, 0 }, { 0, -1 },
			{ 0, 1 } };
	...
	public static void main(String[] args) {
		...
		// Test case 2.
		board = new char[][] { "MA".toCharArray(), "ID".toCharArray(), };

		System.out.println(solution.solve(board));

		// Test case 3.
		board = new char[][] { "DIMAD".toCharArray(), "DIMAI".toCharArray(),
				"DIMAM".toCharArray(), "DDMAA".toCharArray(),
				"AAMID".toCharArray(), };
	...
	public String solve(char[][] board) {
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < m; ++j) {
				if (board[i][j] == 'D') {
					int localCount = dfs(i, j, new boolean[n][m], board, path,
							count);
	...
	private int dfs(int i, int j, boolean[][] visited, char[][] board,
		...
		for (int[] dxdy : this.delta) {
			...
			if (i + dx >= 0 && i + dx < board.length && j + dy >= 0
					&& j + dy < board[i].length
