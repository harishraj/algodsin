...
public class Graph_Coloring {
	...
	/**
	 * Given a graph, check if it can be colored with three colors and no
	 * neighbors of any ponit has the same color.
	 */
		...
		if (color.length == ptr) {
			...
			for (int c = 1; c <= color.length; ++c) {
				if (ptr == 0 || isValidColor(graph, color, ptr, c)) {
					...
					// else, check if it conflics with its neighbor with color c
					// assigned.
					color[ptr] = c;
