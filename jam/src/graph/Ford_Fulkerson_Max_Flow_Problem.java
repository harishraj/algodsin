...
public class Ford_Fulkerson_Max_Flow_Problem {
	...
	public static void main(String[] args) {
		Ford_Fulkerson case1 = new Ford_Fulkerson(new int[][] {
...
class Ford_Fulkerson {
	...
	public void max_flow(int src, int dest) {
		...
		while (this.bfs(residualGraph, src, dest, parents)) {
			// Backtracking the augmenting path from destination to source and
			...
			for (j = dest; j != src; j = parents[j]) {
				...
				residualGraph[i][j] -= path_flow; // Increase flow on forward
													// edge.
				residualGraph[j][i] += path_flow; // Decrease flow on backwrd
													// edge. Augmenting path
													// found later might take
													// reversed direction.
			}
			acc += path_flow;
		}
	...
	// Either dfs or bfs works here. Check if destination is reachable from
	// source.
