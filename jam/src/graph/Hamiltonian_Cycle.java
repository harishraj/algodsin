...
class Hamiltonian_Cycle {
	...
	/**
	 * The question is printing out all possible paths and dfs will serve. You
	 * should check if this graph has hamiltonian path first.
	 */
		...
		if (ptr == path.length) {
			...
			for (int i = 0; i < path.length; ++i) {
				if (ptr == 0 || (!used[i] && graph[path[ptr - 1]][i] == 1)) {
					...
					// or looking for unused point which is connected to
					// previous point.
					path[ptr] = i;
					used[i] = true;
					walk(graph, path, used, ptr + 1);
					used[i] = false; // Backtracking and reset the visited
										// status.
				}
			}
		}
