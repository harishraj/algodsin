...
public class Minimum_Distance_Sum_To_K_Points_In_Maze {
	...
	public static void main(String[] args) {
		...
		Collections.addAll(points, new Point(1, 1), new Point(5, 4), new Point(
				6, 1));
	...
	private void bfs(Point start) {
		...
		while (!curLvl.isEmpty()) {
			...
			// Go up.
			if (p.x - 1 >= 0 && !this.visited[p.x - 1][p.y]
					&& this.maze[p.x - 1][p.y] != 1) {
				nextLvl.add(new Point(p.x - 1, p.y));
			}
			// Go down.
			if (p.x + 1 < this.n && !this.visited[p.x + 1][p.y]
					&& this.maze[p.x + 1][p.y] != 1) {
				nextLvl.add(new Point(p.x + 1, p.y));
			}
			// Go left.
			if (p.y - 1 >= 0 && !this.visited[p.x][p.y - 1]
					&& this.maze[p.x][p.y - 1] != 1) {
				nextLvl.add(new Point(p.x, p.y - 1));
			}
			// Go right.
			if (p.y + 1 < this.m && !this.visited[p.x][p.y + 1]
					&& this.maze[p.x][p.y + 1] != 1) {
				nextLvl.add(new Point(p.x, p.y + 1));
			}
