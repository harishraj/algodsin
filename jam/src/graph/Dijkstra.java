...
public class Dijkstra {
	...
	public static void dijkstra(int[][] graph, int src) {
		...
		for (int count = 0; count < len - 1; ++count) {
			...
			/*
			 * Not sure this part of code will be hit or not. Reason: in method
			 * minDistance the if condition "... && dist[v] < min" will never be
			 * true if dist[v] == MAX_VALUE because init value of min itself is
			 * MAX_VALUE.
			 * 
			 * if (dist[u] == Integer.MAX_VALUE) { continue; }
			 */

			// If we've known the cost from src to u, update if from src to u
			// then
